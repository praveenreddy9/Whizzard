...
export default class StartShiftScreen extends Component {
    ...
    componentDidMount() {
        ...
        this._subscribe = this.props.navigation.addListener('didFocus', () => {
            ...
            self.setState({
                CurrentShiftId,UserFlow})
            self.StartShiftDetails(UserFlow);
        });
    }
    ...
    errorHandling(error) {
        console.log('start shift errror',error,error.response);
        const self = this;
        if (error.response) {
                ...
                if (response.status === 200) {
                     if (response.data != null) {
                        ...
                        self.setState({
                            ...
                            refreshing: false,
                            spinnerBool: false,
                            requireOdometerDetails: StartShiftDetails.userRole === 5 || StartShiftDetails.userRole === 10,
                            requirePackageDetails: (StartShiftDetails.userRole === 1 || StartShiftDetails.userRole === 10) && !StartShiftDetails.hyperLocalOrdersSite,
...
//Odometer reading validate
    ...
    ValidateShiftStart() {
        const {latitude,longitude,UserFlow,StartShiftDetails} = this.state;
        let body = {};
        if (longitude && latitude) {
            if (this.state.selectedTripValue === "NO_LOAD") {
                {
                    UserFlow === 'SITE_ADMIN' || UserFlow === 'NORMAL_ADHOC_FLOW' || UserFlow === 'ADMIN_ADHOC_FLOW'
                        ?
                        body = JSON.stringify({
    ...
    ReadingsCountSTART = (body) => {
        const {UserFlow,CurrentShiftId} = this.state;
        const self = this;
      let apiURL;
        if (this.state.selectedTripValue === "NO_LOAD") {
            if (UserFlow === 'SITE_ADMIN' || UserFlow === 'NORMAL_ADHOC_FLOW' || UserFlow === 'ADMIN_ADHOC_FLOW') {
         ...
         self.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(apiURL, "PUT", body, function (response) {
                if (response.status === 200) {
                    ...
                    if ((UserFlow === 'NORMAL'  ) && self.state.selectedTripValue !== "NO_LOAD") {
   ...
   requestCurrentLocation = async()=>{
    ...
    render() {
        ...
        const {checkOdometerReading,requireOdometerDetails,requirePackageDetails,StartShiftDetails} =this.state;
        return (
                                        ...
                                        this.state.selectedTripValue === "NO_LOAD"
                                            ...
                                            StartShiftDetails.userRole === 1
                                                ...
                                                this.swipeButtonStartShift(!((StartShiftDetails.requireClientLoginId ? this.state.searchPhoneNumber.length === 10  &&
                                                    (this.state.phoneNumberSearchData ? this.state.phoneNumberSearchData.existedUser : false) : true) && (StartShiftDetails.hyperLocalOrdersSite ?true: this.state.PackagesDisplay === true)))
                                                :
                                                StartShiftDetails.userRole === 5
                                                    ?
                                                    this.swipeButtonStartShift(this.state.defaultOdometer !== true)
                                                    ...
                                                    StartShiftDetails.userRole === 10
                                                        ?
                                                        this.swipeButtonStartShift(!((StartShiftDetails.requireClientLoginId ? this.state.searchPhoneNumber.length === 10  &&
                                                            (this.state.phoneNumberSearchData ? this.state.phoneNumberSearchData.existedUser : false) :true) && this.state.defaultOdometer === true && (StartShiftDetails.hyperLocalOrdersSite ?true: this.state.PackagesDisplay === true)))
                                                        :
                                                        StartShiftDetails.userRole >= 15
                                                            ?
                                                            this.swipeButtonStartShift(false)
