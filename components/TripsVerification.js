...
TripsVerification extends React.Component {
    ...
    clearUpdatedValues(statusCheck) {
        ...
        return (
            this.setState({
                ...
                vehicleImageDetailsUpdated:testValue,
                tempVehicleVerificationImageUrl: '',
                tempVehicleVerificationImageFormData: ''
            })
        )
    ...
    rejectTripDetails(reasons) {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(apiURL, "PUT", body, (response) => {
                if (response.status === 200) {
                    let tempNextCard = self.state.currentCardCount + 1
                    self.setState({
                        spinnerBool: false,
                            ...
                            <View style={[Styles.flex1, Styles.p10]}>
                                ...
                                <ScrollView persistentScrollbar={true}>
                                    <View
                                        ...
                                        {
                                            card.requireVehicleImageDetails
                                                ?
                                                <TouchableOpacity
                                                    onPress={() => {
                                                        this.setState({tempCardIndex: index, cardIndex: index}, () => {
                                                            this.useSelectedDataReport(card, 'VEHICLE_IMAGE_DETAILS')
                                                        })
                                                    }}
                                                    activeOpacity={0.7}
                                                    style={[Styles.row, Styles.mTop5]}>
                                                    <View
                                                        style={[Styles.bw1, vehicleImageDetailsUpdated ? Styles.bcAsh : Styles.bcLightRed, Styles.br5, Styles.marH5, Styles.mBtm10]}>
                                                        <View
                                                            style={[vehicleImageDetailsUpdated ? Styles.bgLBlueWhite : Styles.bgLPink, Styles.padV10, Styles.alignCenter, {
                                                                width: Dimensions.get('window').width / 3.8,
                                                                height: 55
                                                            }]}>
                                                            <Text
                                                                numberOfLines={1}
                                                                style={[Styles.f14, vehicleImageDetailsUpdated ? Styles.cLightNavyBlue : Styles.cLightRed, Styles.aslCenter, Styles.ffRRegular]}>{vehicleImageDetailsUpdated ? 'verified' : '--'}</Text>
                                                        </View>
                                                        <View style={[Styles.bgWhite, Styles.padV10, Styles.aslCenter, {
                                                            height: 36
                                                        }]}>
                                                            <Text
                                                                style={[Styles.f12, Styles.cLightBlue, Styles.aslCenter, Styles.ffRRegular]}>Vehicle Details</Text>
                                                        </View>
                                                        <View style={[Styles.posAbsolute, {
                                                            top: 46,
                                                            left: Dimensions.get('window').width / 8.5
                                                        }]}>
                                                            {Services.returnCardStatusIcon(vehicleImageDetailsUpdated)}
                                                        </View>
                                                    </View>
                                                </TouchableOpacity>
                                                :
                                                    ...
                                                    onPress={() => {
                                                        if (card.role === 10) {
                                                            if ((card.attrs.requiresClientUserId === 'true' ? (clientUserIdDetailsUpdated) : true) &&
                                                                ...
                                                                (card.requireVehicleImageDetails ? vehicleImageDetailsUpdated : true) &&
    ...
    useSelectedDataReport(item, selectedButton) {
        this.setState({
            ...
            if (item.unRegisteredUserAdhocShift){
                this.setState({tempShiftVehicleImageUrl:item.shiftVehicleImageDetails ? item.shiftVehicleImageDetails.imageUploadedUrl : null,
                    tempVehicleVerificationImageUrl:item.verificationVehicleImageUrl ? item.verificationVehicleImageUrl :
                        item.verificationVehicleImageDetails ? item.verificationVehicleImageDetails.imageUploadedUrl :null })
            }
        })
    };
    ...
    storeFinalValues() {
        const {
            ...
            finalVehicleVerificationImageUrl,tempVehicleVerificationImageUrl,
            //lite user bank info
        ...
        if (tempData.unRegisteredUserAdhocShift){
            if (vehicleImageDetailsUpdated) {
                tempData.verificationVehicleImageUrl = vehicleImageDetailsUpdated ? finalVehicleVerificationImageUrl : tempVehicleVerificationImageUrl
            }
        }
    ...
    imagesUpload(uploadType) {
        const self = this;
        const {imageType} = self.state;
        Services.checkImageUploadPermissions(uploadType, (response) => {
            ...
            let apiURL = Config.routes.BASE_URL + Config.routes.UPLOAD_VERIFICATION_VEHICLE_SHIFT_IMAGE + 'tripId=' + self.state.selectedCardTripDetails.id;
            const body = formData;
            self.setState({spinnerBool: true}, () => {
                Services.AuthProfileHTTPRequest(apiURL, 'POST', body, function (response) {
                    if (response.status === 200) {
                        self.setState({
                            tempVehicleVerificationImageUrl: image.path,
                            tempVehicleVerificationImageFormData: formData,
                            spinnerBool: false
                        }, () => {
                                        ...
                                        selectedCardTripDetails
                                            ...
                                            <View
                                                ...
                                                <ScrollView
                                                    ...
                                                    {/*LITE USER VEHICLE DETAILS*/}
                                                    {
                                                        selectedCardTripDetails.requireVehicleImageDetails
                                                            ?
                                                            <View>
                                                                <TouchableOpacity
                                                                    activeOpacity={0.7}
                                                                    onPress={() => {
                                                                        this.useSelectedDataReport(selectedCardTripDetails, 'VEHICLE_IMAGE_DETAILS')
                                                                    }}
                                                                    style={[Styles.row, Styles.padV20, Styles.padH15, Styles.jSpaceBet, Styles.bgLightWhite, Styles.OrdersScreenCardshadow]}>
                                                                    <View/>
                                                                    <Text
                                                                        style={[editButton === 'VEHICLE_IMAGE_DETAILS' ? Styles.fWbold : null, Styles.ffRRegular, Styles.f14, Styles.cBlack68]}>Vehicle Details</Text>
                                                                    <View style={[Styles.aslCenter, Styles.padH5]}>
                                                                        {Services.returnCardStatusIcon(vehicleImageDetailsUpdated)}
                                                                    </View>
                                                                </TouchableOpacity>
                                                                                            ...
                                                                                            editButton === 'CLIENT_LOGIN_ID'
                                                                                                ...
                                                                                                editButton === 'PAYMENT_PLAN'
                                                                                                    ...
                                                                                                    editButton === 'OPERATIONS_TYPE'
                                                                                                        ...
                                                                                                        :
                                                                                                        editButton === 'VEHICLE_IMAGE_DETAILS'
                                                                                                            ?
                                                                                                            <View
                                                                                                                style={[Styles.flex1, Styles.p15]}>
                                                                                                                <Text
                                                                                                                    style={[Styles.ffRLight, Styles.f18, Styles.cBlack87]}>Vehicle Details</Text>
                                                                                                                <ScrollView
                                                                                                                    style={[{height: subEditHeightBy60 - 100}]}>

                                                                                                                    <View>
                                                                                                                        <Text
                                                                                                                            style={[Styles.ffRRegular, Styles.f14, Styles.pTop15, Styles.cGrey4F]}>Shift Uploaded Image</Text>
                                                                                                                        <View>
                                                                                                                            <TouchableOpacity
                                                                                                                                style={[Styles.row, Styles.aslCenter]}
                                                                                                                                disabled={!this.state.tempShiftVehicleImageUrl}
                                                                                                                                onPress={() => {
                                                                                                                                    this.setState({
                                                                                                                                        imagePreview: true,
                                                                                                                                        imagePreviewURL: this.state.tempShiftVehicleImageUrl
                                                                                                                                    })
                                                                                                                                }}>
                                                                                                                                <View>
                                                                                                                                    {
                                                                                                                                        this.state.tempShiftVehicleImageUrl
                                                                                                                                            ?
                                                                                                                                            <Image
                                                                                                                                                onLoadStart={() => this.setState({imageLoading: true})}
                                                                                                                                                onLoadEnd={() => this.setState({imageLoading: false})}
                                                                                                                                                ...
                                                                                                                                                source={{uri: this.state.tempShiftVehicleImageUrl}}
                                                                                                                                            />
                                                                                                                                            :
                                                                                                                                            <Text
                                                                                                                                                style={[{
                                                                                                                                                    width: windowWidth / 2,
                                                                                                                                                    height: 98
                                                                                                                                                }, Styles.txtAlignCen,Styles.bgLYellow,Styles.ImgResizeModeStretch]}>NO IMAGE</Text>
                                                                                                                                    }
                                                                                                                                </View>
                                                                                                                    ...
                                                                                                                    <View>
                                                                                                                        <Text
                                                                                                                            style={[Styles.ffRRegular, Styles.f14, Styles.pTop10, Styles.cGrey33]}>Verification Image</Text>
                                                                                                                        <View>
                                                                                                                            <View
                                                                                                                                style={[Styles.row, Styles.aitCenter, Styles.marV5,]}>
                                                                                                                                <TouchableOpacity
                                                                                                                                    disabled={this.state.tempVehicleVerificationImageUrl}
                                                                                                                                    onPress={() => {
                                                                                                                                        this.setState({imageType: 'VEHICLE_FRONT_IMAGE'}, () => {
                                                                                                                                            this.imagesUpload('CAMERA')
                                                                                                                                        })
                                                                                                                                    }}
                                                                                                                                    activeOpacity={0.7}
                                                                                                                                    ...
                                                                                                                                    {LoadSVG.cameraPic}
                                                                                                                                    <Text
                                                                                                                                        style={[Styles.f16, this.state.tempVehicleVerificationImageUrl ? Styles.cDisabled : Styles.colorBlue, Styles.ffLBold, Styles.pRight15]}>Vehicle Photo{Services.returnRedStart()}</Text>
                                                                                                                                </TouchableOpacity>
                                                                                                                                {
                                                                                                                                    this.state.tempVehicleVerificationImageUrl && this.state.showUnVerifiedTripData
                                                                                                                                        ?
                                                                                                                                        <MaterialIcons
                                                                                                                                            onPress={() => {
                                                                                                                                                this.setState({
                                                                                                                            ...
                                                                                                                            {
                                                                                                                                this.state.tempVehicleVerificationImageUrl
                                                                                                                                    ?
                                                                                                                                    <View
                                                                                                                                        ...
                                                                                                                                        <TouchableOpacity
                                                                                                                                            ...
                                                                                                                                            onPress={() => {
                                                                                                                                                this.setState({
                                                                                                                                                    imagePreview: true,
                                                                                                                                                    imagePreviewURL: this.state.tempVehicleVerificationImageUrl
                                                                                                                                                })
                                                                                                                                            }}>
                                                                                                                                            <Image
                                                                                                                                                onLoadStart={() => this.setState({imageLoading: true})}
                                                                                                                                                ...
                                                                                                                                                source={this.state.tempVehicleVerificationImageUrl ? {uri: this.state.tempVehicleVerificationImageUrl} : null}
                                                                                                                                            />
                                                                                                                    ...
                                                                                                                    !this.state.showUnVerifiedTripData
                                                                                                                        ...
                                                                                                                        <View
                                                                                                                            ...
                                                                                                                            <TouchableOpacity
                                                                                                                                onPress={() => {
                                                                                                                                    let resp = {}
                                                                                                                                    resp = Utils.isValueSelected(this.state.tempVehicleVerificationImageUrl, 'Please Upload Vehicle Image');
                                                                                                                                    if (resp.status === true) {

                                                                                                                                        this.setState({
                                                                                                                                            finalVehicleVerificationImageUrl: this.state.tempVehicleVerificationImageUrl,
                                                                                                                                            vehicleImageDetailsUpdated: true,
                                                                                                                                            selectedButton: 'DONE'
                                                                                                                                        }, () => {
                                                                                                                                            this.storeFinalValues()
                                                                                                                                        })
                            ...
                            <View style={[Styles.flex1]}>
                                {
                                    this.state.pendingDatesInfo.length === 0
                                        ?
                                        <View style={[Styles.flex1, Styles.aitCenter, Styles.jCenter]}>
                                            <Text style={[Styles.ffRBold, Styles.f18, Styles.alignCenter]}>No Shifts Found..</Text>
                                        </View>
                                        :
                                        this.state.pendingDatesInfo.map((item,index)=>{
                                            return(
                                                <View key={index}>
                                                    <TouchableOpacity
                                                        onPress={() => {
