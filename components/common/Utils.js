...
Utils.prototype.isValid4DigitOTP = function (value) {
    let response = {};
    if (value) {
        if (value === '...809'){
            response.status = true;
            ...
            // value = value;
            if (/^\d{4}$/.test(Number(value)) && value.toString().length === 4) {
                response.status = true;
                response.message = value;
            } else if (value.toString().length < 4) {
                response.status = false;
                response.message = 'Please Enter 4 Digit OTP code';
            } else {
                response.status = false;
                response.message = 'Please Enter valid 4 Digit OTP code';
            }
        }
    } else {
        response.status = false;
